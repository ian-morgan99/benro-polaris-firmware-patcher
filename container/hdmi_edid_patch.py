...
Idempotent: detects an already-patched input and reports whether output differs."""
import hashlib, sys
...
def main(inp, edid_path, outp):
    ...
    if cur == edid:
        print("already patched: EDID at", hex(EDID_OFF), "matches input EDID exactly")
    ...
    diff = sum(a != b for a, b in zip(cur, edid))
    data[EDID_OFF:EDID_OFF+256] = edid
    open(outp, 'wb').write(data)
    print(f"patched: replaced EDID ({diff}/256 bytes differ from previous)")
