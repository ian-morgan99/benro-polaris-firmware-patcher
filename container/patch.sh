...
# Phase 3 cellular (optional): when the host binds a directory here, the four
# pre-built kernel modules inside (usbserial.ko, option.ko, cdc_acm.ko,
# qcserial.ko) are copied into /app/komod/ and the loader/unloader shell
# scripts are placed alongside the existing sp_usb2net_*.sh pair. When unset,
# the loader/unloader scripts are still placed so a future rebuild can wire
# them up without re-patching the firmware. See docs/CELLULAR.md.
CELLULAR_MODULES_DIR="${CELLULAR_MODULES_DIR:-}"
...
# ---------------------------------------------------------------------------
# 7a. Cellular (Phase 3, optional). Inject pre-built USB-serial host drivers
#     for Quectel / Sierra modems and place a VID-gated loader next to the
#     existing sp_usb2net_*.sh pair. Behaviour:
#
#       - CELLULAR_MODULES_DIR unset OR empty:
#           warn + still place the loader/unloader scripts so they can be
#           wired up by a future rebuild without re-patching the firmware.
#
#       - CELLULAR_MODULES_DIR set, all four .ko files present:
#           copy each .ko into $APP/komod/ with stock perm/uid/gid (matches
#           the stock 0750 / app user used for every other .ko), and place
#           the two scripts alongside.
#
#       - CELLULAR_MODULES_DIR set but modules missing:
#           warn + skip the .ko copy (the loader is still placed, but it
#           will be a runtime no-op).  We never fail-closed here: the user
#           may have intended to ship only the scripts and add the .ko's
#           in a follow-up.
# ---------------------------------------------------------------------------
KOMOD="$APP/komod"                                   # stock location for .ko's
# Borrow the uid/gid/mode of any stock module so the new files are
# indistinguishable from the existing ones on the device.  Using the
# udc-core.ko as the reference: it's small, present in every build, and
# has the canonical 0750 mode.
KO_REF="$KOMOD/udc-core.ko"
if [ ! -f "$KO_REF" ]; then
  # Fall back to the first .ko in the directory if the reference is gone.
  KO_REF="$(find "$KOMOD" -maxdepth 1 -name '*.ko' | head -1)"
fi
[ -f "$KO_REF" ] || die "no reference .ko found in $KOMOD (cannot preserve stock perms)"
K_UID="$(stat -c %u "$KO_REF")"; K_GID="$(stat -c %g "$KO_REF")"; K_MODE="$(stat -c %a "$KO_REF")"

# Scripts always land in $KOMOD next to the existing sp_usb2net_*.sh pair.
# Stock scripts there are mode 0750; we match that.
SH_REF="$KOMOD/sp_usb2net_load.sh"
[ -f "$SH_REF" ] || SH_REF="$(find "$KOMOD" -maxdepth 1 -name 'sp_usb2net_load.sh')"
if [ -f "$SH_REF" ]; then
  S_UID="$(stat -c %u "$SH_REF")"; S_GID="$(stat -c %g "$SH_REF")"; S_MODE="$(stat -c %a "$SH_REF")"
else
  S_UID="$K_UID"; S_GID="$K_GID"; S_MODE=755
fi

install -m "$S_MODE" -o "$S_UID" -g "$S_GID" /opt/patcher/cellular_load.sh   "$KOMOD/cellular_load.sh"
install -m "$S_MODE" -o "$S_UID" -g "$S_GID" /opt/patcher/cellular_unload.sh "$KOMOD/cellular_unload.sh"
log "  placed cellular loader/unloader scripts in $KOMOD"

if [ -z "$CELLULAR_MODULES_DIR" ]; then
  warn "CELLULAR_MODULES_DIR not set -- shipping loader scripts only. The cellular loader will be a runtime no-op until pre-built .ko modules are added at /app/komod/{usbserial,option,cdc_acm,qcserial}.ko."
elif [ ! -d "$CELLULAR_MODULES_DIR" ]; then
  warn "CELLULAR_MODULES_DIR=$CELLULAR_MODULES_DIR is not a directory -- skipping .ko injection (loader scripts already placed)."
else
  INJECTED=0
  MISSING=()
  for mod in usbserial option cdc_acm qcserial; do
    src="$CELLULAR_MODULES_DIR/${mod}.ko"
    if [ -f "$src" ]; then
      install -m "$K_MODE" -o "$K_UID" -g "$K_GID" "$src" "$KOMOD/${mod}.ko"
      log "  injected cellular .ko: ${mod}.ko (mode=$K_MODE uid=$K_UID gid=$K_GID)"
      INJECTED=$((INJECTED+1))
    else
      MISSING+=("${mod}.ko")
    fi
  done
  if [ "$INJECTED" -eq 4 ]; then
    log "cellular: all 4 modules injected ($INJECTED/4). Loader will insmod them at next boot when a Quectel/Sierra modem is detected."
  elif [ "$INJECTED" -gt 0 ]; then
    warn "cellular: only $INJECTED/4 modules present in $CELLULAR_MODULES_DIR (missing: ${MISSING[*]}). The loader will skip any missing file; cellular data may not come up."
  else
    warn "cellular: $CELLULAR_MODULES_DIR has none of the expected .ko files (looking for usbserial.ko, option.ko, cdc_acm.ko, qcserial.ko). Loader scripts placed, but no modules will be inserted at boot."
  fi
fi

# Wire the cellular loader into /app/bootapp so it runs once per boot, after
# the HiSilicon platform modules are up.  We use a guarded insertion: only
# add the line if cellular_load.sh is present in $KOMOD (always true after
# the install above) AND bootapp currently does not already reference it
# (idempotent on re-patch).
BOOTAPP="$APP/bootapp"
if [ -f "$BOOTAPP" ] && [ -x "$KOMOD/cellular_load.sh" ]; then
  if grep -q 'cellular_load.sh' "$BOOTAPP"; then
    log "bootapp: cellular_load.sh invocation already present, leaving as-is"
  else
    # Stock bootapp runs `cd /app/komod && ./sp_load3559v200 -i` once on boot.
    # Mirror the same `cd && ./` idiom so the new line is syntactically a
    # drop-in sibling of the existing one.  Only insert when the marker
    # pattern is found, otherwise the file is in an unexpected shape and
    # we fall back to a loud warning rather than a silent no-op.
    if grep -qE '^cd /app/komod[[:space:]]*&&[[:space:]]*\./sp_load3559v200' "$BOOTAPP"; then
      # Insert the cellular loader line directly after the sp_load line so
      # modules are loaded in the same order as the stock platform init.
      python3 - "$BOOTAPP" <<'PY'
import sys, pathlib
p = pathlib.Path(sys.argv[1])
text = p.read_text()
needle = "cd /app/komod && ./sp_load3559v200 -i"
add    = "cd /app/komod && ./cellular_load.sh"
if needle not in text or add in text:
    sys.exit(0)
# Match the line, and capture its trailing newline faithfully.
...
PY
      if [ $? -eq 0 ] && grep -q 'cellular_load.sh' "$BOOTAPP"; then
        log "bootapp: wired './cellular_load.sh' invocation directly after sp_load3559v200 -i"
      else
        warn "bootapp: failed to inject cellular_load.sh line — you'll need to wire it manually"
      fi
    else
      warn "bootapp: could not find 'cd /app/komod && ./sp_load3559v200 -i' line to anchor the cellular loader; not modifying bootapp"
    fi
  fi
else
  log "bootapp: no $BOOTAPP or cellular_load.sh not present — skipping bootapp wiring"
fi
