...
## Appendix: the stock Benro `firmwareInfo` is itself internally inconsistent

While stress-testing `verify_firmwareinfo.py` against real artifacts
(2026-08-28, in this session), we ran the verifier against the **unmodified
Benro stock** `FwPkt.zip` (md5 `fd8147c9…`, straight from Benro's official
download). The result is a single mismatch on the `appfs` entry:

```
appfs      MISMATCH  /tmp/original-extract/FwPkt/camera/appfs.ubifs
  size: 64356352 (claimed 64356352)
  md5:  91629acf0494b7f43298f6821913124f (claimed 1775c7bc4eee7d549a36fa28bb13f367)
```

That is, Benro's own `firmwareInfo` claims `appfs` MD5
`1775c7bc4eee7d549a36fa28bb13f367`, but the `appfs.ubifs` Benro ships has
MD5 `91629acf0494b7f43298f6821913124f`. The size is unchanged, and every
other entry (config, uImage, rootfs, polaris403, polaris413) is internally
consistent. So Benro's stock FwPkt is **itself** a candidate for the
silent-reject failure mode.
...
2. **We do not know whether the Polaris ignores a known-bad `appfs` MD5
   on stock** (which would explain how Benro's own product works), or
   whether the on-board updater is lenient about all `appfs` MD5s, or
   whether some Benro build step legitimately produces a different
   artifact from the one we see. The mystery is not for us to solve —
   but the lesson is: regenerate `firmwareInfo` from the bytes you
   actually ship, every time.
3. **The integrity gate (`verify_firmwareinfo.py` in `patch.sh`) is
...
The implication for Benro support: if you ever see a Polaris that
silently rejects a *stock* Benro firmware, this is the first place to
look. The MD5 in the stock `firmwareInfo` is wrong.
