...
| **o-v9o-capture-shim** | local build `/tmp/bpp-clean-v9o/out/o-v9o-capture-shim-20260916/FwPkt.zip` (PrivateResearch upload pending canary) | `27cead5f8c8294bfb764ca57e9bd77cb` | `93b43c8cc4c33ff1` | `34c1f07580c6cc2b4a1b5c83d974416d` | `121675124e173da1864421acebea8e20c851c827` | patcher @ `fa10bfb/fix/release-safety-gates` (clean tree, `patcher_dirty_diff_hash=` empty) | **INSTALLED 2026-09-16; no-camera canary in progress** | Adds SHIM #6: `gp_camera_capture` (shutter path) wrapped with the same non-blocking preview-backoff cooldown as SHIM #5, so a failing Pentax camera backs off on BOTH preview and still-capture traffic (issue #93/#55); plus `port_owned_by()` 8080 ownership proof in `restart_gphoto.sh` and PID-reuse-safe lock-ownership validation in the USB supervisor. Same libgphoto2 source as o-v9n; built under the fail-closed dirty-patcher gate with `build_id=6.0.0.54.6`; structural + firmwareInfo manifest gates passed (all 7 payloads MD5+size verified on-card before reboot). Installed via sanctioned extracted-`FwPkt/` SD flow + `/sbin/reboot`; post-boot provenance `patcher_commit=fa10bfb…`, `build_id=6.0.0.54.6` verified on device; live loader carries the capture-shim strings. **No-camera canary PASSED 2026-09-16** (~48 min soak, T0 18:46 / T1 19:02 / T2 19:34 UTC): pgphoto PID 250 stable across all three samples (no crash loop), both listeners up (8080 pgphoto / 9090 polestar_app), zero FATAL/state:-/ret:- in Clog at every sample, load steady ~1.0; init-block count churn (78→44→0) = Clog rotation, not a crash loop. Camera baselines (roadmap steps 6/7) still blocked on camera availability. |
