...
| Camera batteries | **K-1 II and K-3 III both had their batteries changed (2026-09-10)** — the "battery is the constraint (last 1–2 h)" note from `docs/HANDOVER-2026-09-10.md` §1/§7 no longer applies; direct-on-PC baselines (§3b) are no longer time-boxed by battery |
| Last reboot attempt | today, 16:0x UTC — AP appeared briefly, sshd never bound, dropped before SSH connect |
| Current state | **v3 K-1II/K-3III build installed (2026-09-07 22:15 UTC); device healthy, camera unplugged.** The v3 tree (`out/k1ii-k3iii-fixed-v3/`, payload appfs MD5 `d220682e…`) was zipped by an agent session (outer MD5 `5491835a…`), hand-copied to the SD card, and installed via the on-boot watcher. Confirmed on-device: `/app/lib/stage2/*` = Sep 7 22:15, fresh libgphoto2 2.5.34 core at stage2 + stock paths, `ptp2.so` carries Pentax markers. A K-1 II PTP lockup (stale-session `-2` loop) took down the network stack; a full reboot cleared it and BT-wake via iPhone restored SSH (`root@192.168.0.1`). Provenance gap from the hand-zipped tree is now closed by `docs/FWPKT-PROVENANCE-CONTRACT.md` (registry + cross-repo/cross-agent handoff rule). See [§ Session log 2026-09-07](#session-log-2026-09-07--direct-nand-edits-bypass-bricks-gimbal-sd-card-extract-fw-recovery) and `docs/RUN-JOURNAL.md` §"2026-09-07/08". |
